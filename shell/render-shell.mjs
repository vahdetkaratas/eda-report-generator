...
function linkItems(items) {
  return items
    .map((item) => {
      ...
      const descHtml = desc
        ? `\n<span class="art-sidebar-link-desc">${escapeHtml(desc)}</span>`
        : "";
      return `<li><a href="${escapeHtml(item.href)}"${ext}>${escapeHtml(item.label)}</a>${descHtml}</li>`;
    })
    .join("\n");
}
...
async function main() {
  ...
  const logo1 = project.logoLine1 || "EDA";
  const logo2 = project.logoLine2 || "Report";
  const heroTitle = project.heroTitle || project.title;
  const footerTitle = project.footerProjectName || "EDA Report Generator";
  const footerSub =
